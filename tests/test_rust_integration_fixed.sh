...
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "  OpenClaw+ Rust 核心模块集成测试 (修正版)"
...
if [ -f "Cargo.lock" ]; then
    ...
    # 检查关键依赖
    DEPS=("tokio" "serde" "wasmtime" "wasmedge")
...
if [ -f "crates/agent-executor/Cargo.toml" ]; then
    if grep -q "security\|inference\|plugin" "crates/agent-executor/Cargo.toml"; then
...
if [ -f "crates/plugin/Cargo.toml" ]; then
    if grep -q "agent-executor\|security" "crates/plugin/Cargo.toml"; then
        log_ok "  ✓ 插件有内部依赖"
    fi
fi
...
if [ -d "crates/inference" ]; then
    # 检查推理模块的关键功能
    if [ -f "crates/inference/src/lib.rs" ]; then
        if grep -q "Ollama\|ollama" "crates/inference/src/lib.rs"; then
            log_ok "  ✓ Ollama 集成存在"
        fi
        
        if grep -q "Inference\|inference" "crates/inference/src/lib.rs"; then
            log_ok "  ✓ 推理接口存在"
        fi
    fi
    PASSED_TESTS=$((PASSED_TESTS + 1))
...
log_info "测试安全模块集成"
if [ -d "crates/security" ]; then
    if [ -f "crates/security/src/lib.rs" ]; then
        if grep -q "Security\|security" "crates/security/src/lib.rs"; then
            log_ok "  ✓ 安全接口存在"
        fi
    fi
    PASSED_TESTS=$((PASSED_TESTS + 1))
...
if [ -d "crates/sandbox" ]; then
    if [ -f "crates/sandbox/src/lib.rs" ]; then
        if grep -q "WasmEdge\|wasmtime" "crates/sandbox/src/lib.rs"; then
            log_ok "  ✓ WasmEdge/Wasmtime 集成存在"
        fi
        
        if grep -q "Sandbox\|sandbox" "crates/sandbox/src/lib.rs"; then
            log_ok "  ✓ 沙箱接口存在"
        fi
    fi
    PASSED_TESTS=$((PASSED_TESTS + 1))
...
log_info "检查插件相关模块"
PLUGIN_CRATES=("plugin" "plugin-sdk" "wasm-plugin")
for crate in "${PLUGIN_CRATES[@]}"; do
    if [ -d "crates/${crate}" ]; then
        log_ok "  ✓ ${crate} 模块存在"
        
        if [ -f "crates/${crate}/src/lib.rs" ]; then
            if grep -q "Plugin\|plugin" "crates/${crate}/src/lib.rs"; then
                log_ok "    ✓ 包含插件接口"
            fi
        fi
    else
        log_warn "  ⚠ ${crate} 模块不存在"
    fi
done

log_info "检查插件集成"
if [ -f "crates/plugin/src/router.rs" ]; then
    if grep -q "skill_execute\|execute" "crates/plugin/src/router.rs"; then
        log_ok "  ✓ 技能执行路由存在"
    fi
fi
...
# 保存测试报告
REPORT_FILE="${PROJECT_ROOT}/RUST_INTEGRATION_FIXED_REPORT_$(date +%Y%m%d_%H%M%S).txt"
{
    echo "OpenClaw+ Rust 核心模块集成测试报告 (修正版)"
